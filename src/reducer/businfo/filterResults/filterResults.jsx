...
export const filterReducer = (state, action) => {
  switch (action.type) {
    case DEPART_BEFORE_6:
      if (action.apply) {
        return {
          ...
          filteredBuses: state.filteredBuses.filter(bus =>
        ...
        return {
          ...state,
          filters: state.filters.filter(filter => filter.type !== action.type)
        };
      }
    case DEPART_6_12:
      if (action.apply) {
    ...
    case TYPE_SLEEPER:
      if (action.apply) {
        return {
          ...state,
          filters: state.filters.concat([action]),
          filteredBuses: state.filteredBuses.filter(bus => bus.sleeperAvailable)
        };
      } else {
        return {
          ...state,
    ...
    case ARRIVE_BEFORE_6:
      if (action.apply) {
        return {
          ...
          filters: state.filters.concat([action]),
          filteredBuses: state.filteredBuses.filter(
            bus => parseInt(bus.arrival.substring(0, 2)) <= 6
          )
        };
      } else {
        return {
    ...
    default:
      ...
      return {
        ...state,
        selectedBus: action.bus ? action.bus : state.selectedBus,
        filteredBuses: bus
      };
  }
};
